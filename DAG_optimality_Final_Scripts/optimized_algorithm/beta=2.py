...
class TOPO_linear:
    ...
    def _h(self, W, eps=1e-20):
        ...
        #H = np.linalg.matrix_power(Id+(1/d)*((W*W).T),self.d-1)
    ...
    def fit(self, X, topo: list, no_large_search, size_small, size_large):
        ...
        while bool(idx_set):
            ...
            if np.any(loss > loss_collections):
                ...
                # print("Using larger space")
                Z0 = np.copy(self.Z)
                np.fill_diagonal(Z0, False)

                G_loss_abs = np.abs(G_loss)

                indx1_small = find_hgrad_index(G_loss_abs, G_h, Z=Z0, thres=0.1* small_thres)
                idx_set_large = set(zip(indx1_small[:, 0], indx1_small[:, 1]))
                ...
                for i in range(idx_len):
                    W_c,topo_c = self._update_topo_linear(W=self.W,topo=self.topo,
                                                                 idx=idx_set[i])
                    loss_c, _ = self.score(X=self.X, W=W_c)
                    loss_collections[i] = loss_c
                ...
                if np.any(loss > loss_collections):
                    ...
                    idx_set_sizes.append(len(idx_set_large))
                    for i in range(idx_len):
                        W_c, mu_c, topo_c = self._update_topo_linear(W=self.W, mu=self.mu, topo=self.topo,
                                                                     idx=idx_set[i])
                        loss_c, _ = self.score(X=self.X, W=W_c, mu=mu_c)
                        loss_collections[i] = loss_c
...
def score(X, W):
    M = X @ W
    R = X - M
    #loss = (1.0 / (X.shape[0]*X.shape[1])) * (R ** 2).sum()
    loss = 0.5 / X.shape[0] * (R ** 2).sum()
    G_loss = - 1.0 / X.shape[0] * X.T @ R
    #G_loss = (2.0 / (X.shape[0]*X.shape[1])) * (R ** 2).sum()
...
def score(X, W):
    ...
    G_loss = - (1.0 / X.shape[0]*X.shape[1]) * X.T @ R  # + 0.01*np.sum(np.sign(W))
...
if __name__ == '__main__':
    ...
    thresholded_B = np.where(np.abs(W_est.T) >= 0.3, W_est.T, 0)
