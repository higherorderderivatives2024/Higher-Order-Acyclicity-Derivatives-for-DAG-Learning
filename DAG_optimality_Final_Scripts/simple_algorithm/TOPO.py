...
def find_hgrad_index(G_h, Z, thres=1e-2):
    ...
    G_h0 = copy(G_h)
    # not0grad = np.logical_or(G_h0 <= (-thres), G_h0 >= thres)
    # not0grad = np.logical_and(G_h0 <= thres)
...
def find_idx_set(G_h, G_loss, Z, size_small, size_large):
    ...
    assert size_large <= d * (
                d - 1) / 2, "please set correct size for large search space, it must be less than d(d-1)/2"
